...
/*-----------------------------------------------------------------------------------
 抜粋をカスタマイズ
-----------------------------------------------------------------------------------*/
function get_custom_excerpt( $content=null, $length=100) {
	global $more;
...
function get_my_post_thumbnail_url($size='full') {
	...
	if (has_post_thumbnail() ){ //アイキャッチ
  	...
  	$eyecatch_url = get_template_directory_uri().'/common/img/eyecatch-dummy.jpg';
	}
	return $eyecatch_url;
}
...
/*-----------------------------------------------------------------------------------
 製品のカテゴリ(大カテゴリ)を取得(リンクなし)
-----------------------------------------------------------------------------------*/
function get_product_parent_category_without_link( $sep = ', ') {
	$str = '';
	$terms = get_the_terms( get_the_ID(), 'product-cat');
	if ( !empty($terms) && !is_wp_error($terms) ) {
		foreach( $terms as $term ) {
      if($term->parent) {
        $parent_term = get_term($term->parent, 'product-cat');
        if(strpos($str,$parent_term->name)=== false) {
          $str .= $parent_term->name . $sep;
        }
      }
      else {
        if(strpos($str,$term->name)=== false) {
          $str .= $term->name . $sep;
        }
      }
		}
...
/*-----------------------------------------------------------------------------------
 特定の固定ページに属しているか？
 （指定したスラッグのページもしくは指定したスラッグの子ページのときtrue）
-----------------------------------------------------------------------------------*/
function is_belog_page($page_slug = '') {
  global $post;
  $ret = false;
 
  if($page_slug != '' && is_page()) {
    $target_page_id =  get_page_by_path($page_slug)->ID;
   if (($post->post_parent ==  $target_page_id ) ||
        ($post->ID ==  $target_page_id)){
      $ret = true;
    }
  }
  return $ret;
